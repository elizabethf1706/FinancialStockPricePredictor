...
def get_tomorrow_forecast(forecast_df):
    """
    Get the forecasted closing price for tomorrow.
    
    Args:
        forecast_df (DataFrame): Output from Prophet's predict() method
        
    Returns:
        float or None: predicted closing price (yhat) for tomorrow
    """
    tomorrow = pd.Timestamp.today() + pd.Timedelta(days=1)
    tomorrow = tomorrow.normalize()  # strip time portion

    row = forecast_df[forecast_df['ds'] == tomorrow]
    
    if not row.empty:
        return row.iloc[0]['yhat']
    else:
        print("Tomorrow's date not found in forecast")
        return None
